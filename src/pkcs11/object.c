			   ...
			   CK_ULONG ulCount)
{
	...
	if (obj == NULL || (pTemplate == NULL && ulCount != 0))
...
void object_free(struct pkcs11_object *obj)
{
	...
	if (obj->keytype == CKK_EC || obj->keytype == CKK_EC_EDWARDS) {
		if (obj->data.ec_ed.ec_params != NULL)
			free(obj->data.ec_ed.ec_params);
		if (obj->data.ec_ed.ec_point != NULL)
			free(obj->data.ec_ed.ec_point);
		if (obj->data.ec_ed.spki != NULL)
			free(obj->data.ec_ed.spki);
		if (obj->data.ec_ed.pkey != NULL)
				 ...
				 EVP_PKEY *pkey)
{
	...
	if (obj->data.ec_ed.spki == NULL && obj->data.ec_ed.spki_len != 0)
		goto err;
			      ...
			      struct pkcs11_object *obj)
{
	...
	for (i = 0; i < ulCount; i++) {
		...
		if (attr->ulValueLen == 0)
			continue;
		if (attr->pValue == NULL || pTemplate[i].pValue == NULL)
			return 0;
