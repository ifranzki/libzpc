			  ...
			  CK_MECHANISM_PTR pMechanism)
{
	...
	if (sess == NULL || key == NULL || pMechanism == NULL)
		     ...
		     CK_BYTE_PTR pSignature, CK_ULONG_PTR pulSignatureLen)
{
	...
	case CKM_ECDSA:
		 ...
		 */
		if (pSignature == NULL || *pulSignatureLen < pkcs11_len) {
			*pulSignatureLen = pkcs11_len;
			return (pSignature == NULL) ?
...
void signature_sign_cleanup(struct pkcs11_session *sess)
{
	...
	if (sess->sign.pkey_ctx != NULL)
		EVP_PKEY_CTX_free(sess->sign.pkey_ctx);
	if (sess->sign.md_ctx != NULL)
			    ...
			    CK_ULONG ulSignatureLen)
{
	...
	if (sess->verify.signature == NULL && ulSignatureLen != 0) {
		       ...
		       CK_BYTE_PTR pSignature, CK_ULONG ulSignatureLen)
{
	...
	if (encoded_signature && ossl_sig != NULL)
...
void signature_verify_cleanup(struct pkcs11_session *sess)
{
	...
	if (sess->verify.pkey_ctx != NULL)
		EVP_PKEY_CTX_free(sess->verify.pkey_ctx);
	if (sess->verify.md_ctx != NULL)
